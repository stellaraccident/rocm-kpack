...
//
// IMPORTANT: For PIE binaries, pointers MUST be initialized with relocatable
// addresses (like &dummy_data) to generate R_X86_64_RELATIVE relocations. The
// elf_modify_load set-pointer tool requires existing relocations for PIE/shared
// libraries and will fail with a clear error if the pointer location has no
// relocation entry.
//
// Constant addresses (like 0x1000) do NOT generate relocations and will cause
// the tool to error out, preventing creation of binaries that would crash at
// runtime.
struct TestWrapper test_wrapper __attribute__((section(".test_wrapper"))) = {
    .magic = 0x54455354,  // "TEST" in little-endian
    .version = 1,
    .data_ptr =
        dummy_data,  // Points to dummy_data, generates relocation in PIE
    .reserved = NULL};
...
int main() {
  ...
  if (test_wrapper.data_ptr == NULL ||
      test_wrapper.data_ptr == (const char*)0x1000) {
  ...
  printf("  Data content: \"");
  for (size_t i = 0; i < expected_len && test_wrapper.data_ptr[i] != '\0';
       i++) {
