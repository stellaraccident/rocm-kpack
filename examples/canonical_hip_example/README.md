...
2. A test case for kernel packing (removing fat binaries and using external device code)
3. A basis for runtime modifications to support kpack'd binaries
4. A validation suite for ensuring packing doesn't break functionality

## Next Steps

After verifying the baseline functionality:
1. Pack the binaries using rocm-kpack tools
2. Modify the binaries to remove `.hip_fatbin` sections
3. Test runtime loading of external device code
4. Iterate on runtime integration until packed binaries function correctly
