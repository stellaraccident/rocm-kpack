"""Utility for neutralizing fat binaries by removing .hip_fatbin sections.
...
Usage:
  python -m rocm_kpack.tools.neutralize_binary <input_binary> <output_binary>

Example:
  python -m rocm_kpack.tools.neutralize_binary librocblas.so.5.1 librocblas_host_only.so
"""
...
def run(args: argparse.Namespace):
    ...
    print(f"Neutralizing: {input_path}")
    print(f"Output: {output_path}")
    print()

    result = neutralize_binary(input_path, output_path, verbose=True)
...
def main(argv: list[str]):
    p = argparse.ArgumentParser(
        description="Neutralize fat binaries by removing .hip_fatbin sections and reclaiming disk space"
    )
